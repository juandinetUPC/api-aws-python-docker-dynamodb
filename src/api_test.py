...
class TestBuildResponse(unittest.TestCase):
    def test_buildResponse(self):
        response = custom_encoder.buildResponse(
            200, {'mensaje': 'hola', 'age': Decimal(5)})
...
class TestCreate(unittest.TestCase):
    def test_create_user(self):
        ...
        if response.status_code == 201:
            self.assertEqual(sorted(response.json()), sorted(
                {'Message': 'Usuario creado', 'User': payload}))
        elif response.status_code == 202:
            self.assertEqual(sorted(response.json()), sorted(
                {'Message': 'Usuario ya existe', 'User': payload}))
...
class TestRead(unittest.TestCase):
    def test_read_user(self):
        ...
        if response.status_code == 200:
            ...
            self.assertEqual(sorted(response.json()), sorted(
                {'username': 'Pepito Perez', 'userid': str(userid), 'age': (29)}))
    ...
    def test_read_user_fail(self):
        ...
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.json(), {
                         'Message': 'userid:99999999 not found'})
